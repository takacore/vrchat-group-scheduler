...
- **バックアップ / 復元**: 予約投稿とグループ投稿権限キャッシュを JSON にエクスポート／インポート。更新時や別PCへの移行でデータを引き継げます。
...
> 更新時のデータ引き継ぎ: v2.2.0-rc.8 以降は固定の拡張機能 ID を使用するため、ZIP を別フォルダへ展開して読み込み直しても、予約投稿・グループ権限キャッシュ・言語設定が自動的に引き継がれます。rc.7 以前から rc.8 への最初の更新だけは、現在 Chrome に読み込ませているフォルダへ上書きして「更新」してください。別PCへ移す場合は、引き続き **Backup / Restore** をご利用ください。
