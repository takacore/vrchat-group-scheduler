...
> 更新時のデータ引き継ぎ: Chrome Web Store 経由の更新では、既存の拡張機能 ID により予約投稿・グループ権限キャッシュ・言語設定が自動的に保持されます。パッケージ化されていない拡張機能では、**同じフォルダに上書きして「更新」**すればデータが保持されます。別フォルダ／別PCへ移す場合は、**Backup / Restore** をご利用ください。
